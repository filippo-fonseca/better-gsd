...
**The backlog — leave ideas for the next sesh.** While a session runs you keep thinking of what to do next. Bank each one with `/bgsd-queue "<idea>"` (or just tell Kiwi "queue that for next sesh"); they pile up in this repo's queue. Each banked idea also **files a GitHub issue** (when the repo has a GitHub remote), so deferred scope is tracked the moment you queue it. When you're done, run a bare `/bgsd-sesh` and the Conductor shows the whole queued batch in a selector, so you multi-pick what to pull into the next session, or ignore it and type a fresh prompt. Pulling an idea in links the work back to its issue: the resulting PR **closes the very issue the idea was filed under**. Deferred scope is never lost and never re-typed. (This queue at `.bgsd/queue` is bgsd's one canonical backlog — not any `.planning/` file.)
