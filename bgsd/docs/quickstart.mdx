...
| `/bgsd-modify-memory "<preference>"` | Save a setting or preference to `BGSD.md` in plain English (a typed flag always overrides it). |
| `/bgsd-recall "<terms>"` | Search past session history and get a conversational answer (read-only). |
| `/bgsd-generate-brief [<run-id>]` | Write a clean markdown brief of a past session (defaults to the last) to `.bgsd/briefs/` to feed into the next one. |
| `/bgsd-clean` | Prune merged bgsd branches and their stale worktrees. `--dry-run` / `--json` preview; `--force` for unmerged (protected branches always safe). |
