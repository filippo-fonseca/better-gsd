...
description: "Feed review findings back into the pipeline (full re-run of both loops, or --fast parallel fix agents with no verification), aggregate per-agent CHANGELOGs, assemble the PR description, and open a real gh pr create — always against a non-default branch, always human-gated behind --live."
---

# Feedback Mode + CHANGELOG → PR

When the User Review Gate returns `request_changes`, `/bgsd-feedback` ingests your findings and re-routes them through the fix pipeline. When you approve, `/bgsd-changelog` aggregates every subagent's changes into a PR description and — under an explicit `--live` opt-in — runs `gh pr create` against a non-default branch. Neither the feedback re-run nor the real PR creation happens without your explicit instruction.
...
flowchart TD
    ...
    M["requireLiveFlag() enforced\nrequireNotDefaultBranch(base) enforced\nnext / main / master refused — NFR-01"] --> N

    N["gh pr create\n--base <non-default-branch>\n--head <rehearsal or feature branch>\n--title <title>\n--body <body>"] --> DONE["PR created\nURL printed"]
```
...
|-------|-------|-----------------|
| `requireNotDefaultBranch(base)` | **Always** (even in dry-run) | PR targeting `next`, `main`, or `master` (NFR-01) |
| `requireLiveFlag()` | Only for live execution | Accidental automation without explicit human opt-in |

The non-default-branch guard fires unconditionally, before the `--live` check. A PR targeting `next`/`main`/`master` is refused even in dry-run mode — this guard cannot be bypassed.
...
======================================================================
  ...
  Base branch : rehearsal/bgsd-0001-my-feature
  Head branch : rehearsal/bgsd-0001-my-feature
  ...
  gh pr create --base rehearsal/bgsd-0001-my-feature ...

  To create the real PR, re-run with --live.
  NFR-01: next/main/master are NEVER the PR base — this guard is always on.
======================================================================
```
...
1. You are NOT on `next`/`main`/`master` — the guard enforces this unconditionally.
...
With `--live` set and a non-default base branch, `liveCreatePr()` runs:

```
gh pr create --base rehearsal/bgsd-0001-my-feature \
             --head rehearsal/bgsd-0001-my-feature \
             --title "bgsd run: bgsd-0001-my-feature — ..." \
             --body "..."
```

The PR URL is printed on success. Only you can merge `rehearsal/<run-id>` into `next`, by hand.
...
|----------|----------|-------------|
| `--run-id <id>` | Yes | The run identifier. |
| `--base <branch>` | Yes | PR base branch. Must NOT be `next`/`main`/`master`. |
| `--head <branch>` | Yes | PR head branch (usually `rehearsal/<run-id>`). |
...
| Rule | Detail |
|------|--------|
| **NFR-01 (branch safety)** | Feedback fixes land on `rehearsal/<run-id>` and worktree branches only. The PR ALWAYS targets a non-default branch. `requireNotDefaultBranch()` fires unconditionally — before `--live`, before any GitHub I/O. Only you merge `rehearsal/<run-id>` into `next`, by hand. |
