...
flowchart TD
    ...
    F --> G["Footer\n🔒 main-protected  integrates on next; main never written by an agent  2026-06-29 14:03:22 UTC"]
```
...
| `executing` | `"3 workers underway, sir — wave 2 of 3. 4/4 agents finished, 2/4 verified and merged into next."` |
| `checkpoint` | `"Kiwi needs your go/no-go at the merge boundary, sir. Please review and respond."` |
| `done` | `"Run complete, sir — 4 units merged into next. The integrated build is ready for your inspection."` |
| `aborted` | `"Run aborted, sir. All branches and control files have been preserved."` |

Alongside this one-line summary, the Conductor narrates the pipeline conversationally during a session: the exact stage and agent names, X/Y progress counts ("4/4 agents finished, 2/4 verified and merged into `next`"), an explanation of each PR and merge, and the exact command to type at every human gate. (This richer narration layer is being wired now.)
...
| `/bgsd-clean-branches` | Prune merged worktree/unit branches already integrated into `next`; never deletes the standing `next` branch or any unmerged branch. |
