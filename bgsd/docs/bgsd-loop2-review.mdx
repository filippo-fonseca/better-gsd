...
description: "Loop 2 integration verify→fix over rehearsal/<run-id> and the User Review Gate: boot the integrated app, run the Integration Tester, fix defects, re-merge, re-verify — then present the human a checklist and a GSD-style selector verdict that is never auto-passed."
---

# Loop 2 + User Review Gate — Integration → Human Handoff

After v2's Conductor assembles `rehearsal/<run-id>` and you approve its merge-boundary checkpoint, v3 takes over. `/bgsd-integrate` (backed by `loop2-live.mjs`) runs **Loop 2** — the integration-level sibling of v1's per-worktree Loop 1, applied to the whole assembled system. When Loop 2 returns a clean `PASS`, the Conductor opens the **User Review Gate** (`/bgsd-user-eval`): boot the rehearsal app, print a checklist, and capture your verdict. Nothing advances automatically. Every step is bounded, reversible, and human-gated behind `--live`.
...
Loop 2 is the **integration-level sibling of v1's Loop 1** — "same loop shape, applied twice" (Plan Part 5). Where Loop 1 ran per-worktree (verifying a single unit's work), Loop 2 runs over the WHOLE assembled `rehearsal/<run-id>` branch, verifying the integrated system across feature boundaries.
...
|-------------------|-------|----------------------------------|
| `verify()` | mock returning fixture report | boot rehearsal app + run Integration Tester |
| `fix(defects)` | mock | spawn parallel fix agents in worktrees off `rehearsal/<run-id>` |
| `reMerge()` | mock | re-integrate via `conflict.mjs` dependency-ordered merge |
...
The `🔒 main-protected` indicator is always visible in the banner.
...
The live path boots the actual `rehearsal/<run-id>` app and drives real Integration-Tester→fix cycles. It is **off by default** and requires an explicit `--live` opt-in.
...
6. **Isolation in place.** `runtime-isolate.sh` isolation is configured: the rehearsal app runs on its own port/DB/env with no shared state with production.
7. **`rehearsal/<run-id>` only.** Fix-agent worktrees branch off `rehearsal/<run-id>`. `next` is NEVER touched automatically.
...
| `--port <n>` | No (default: 3099) | Port to boot the rehearsal app on. |
| `--live` | **Human-gated** | Boot the real rehearsal app. Default is dry-run (no boot). |
...
| Rule | Detail |
|------|--------|
| **NFR-01 (branch safety)** | bgsd never writes to `next`. Fix-agent branches and re-merges all target `rehearsal/<run-id>`. Only you merge `rehearsal/<run-id>` into `next`, by hand. |
...
| **NFR-10 (live integration run is human-gated)** | Real booting and real Tester→fix cycles are exercised only under explicit `--live` opt-in, human-supervised, never in CI, `next` never written. `--dry-run` is the default. |
