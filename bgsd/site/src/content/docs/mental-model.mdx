...
```mermaid
flowchart TD
    S[Start /bgsd-sesh] --> SEL[Choose worker path]
    SEL --> DOC[Doctor validates]
    DOC --> C[Conductor scopes and plans]
    C --> PA[Pipeline Agents in worktrees]
    PA --> V[Verify · evidence]
    V --> C
    C -->|Repair| C
    C -->|Accept| N[Integrate to next]
    N --> H[Human gate · next → main]
    H -->|Feedback / follow-up| S
```

| Phase | Owner |
| --- | --- |
| Path selection + Doctor | You + engine (every **new** session) |
| Scope / seed / route | Conductor |
| Implement in worktrees | Build-lane Pipeline Agents |
| Loop 1 / Loop 2 evidence | Evaluate lane |
| Adjudicate + steer | Conductor |
| `next` → `main` | Human only |
