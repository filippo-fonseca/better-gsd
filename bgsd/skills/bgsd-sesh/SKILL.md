...
The live session model is the Conductor and authors all unit seeds. Do not launch a separate pre-planner. A Quick session is still delegated: the Conductor chooses one or more small direct-work Pipeline Agents, can run them serially or in parallel, and steers them from evidence. Quick workers skip GSD phases, but work in isolated worktrees and own every code edit and repair. The Conductor never edits code in a BGSD session. A direct Conductor edit is allowed only outside BGSD, when the user directly requests a genuinely trivial a-few-lines change. Feature and Project Pipeline Agents use the full GSD workflow; Loop 1/Loop 2 verification and final fresh review use the evaluation lane. Keep every Conductor interaction in the configured Kiwi persona and preserve all branch, verification, and human-gate rules from the Conductor contract.
